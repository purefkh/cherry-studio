...
const GenerateImageButton: FC<Props> = ({ model, mentionModels, ToolbarButton, assistant, onEnableGenerateImage }) => {
  ...
  // Show the button if the current model or any mentioned model supports image generation
  ...
  return (
    <Tooltip
      ...
      <ToolbarButton type="text" disabled={!supportGenerateImage} onClick={onEnableGenerateImage}>
        <Image
          size={18}
          color={supportGenerateImage && assistant.enableGenerateImage ? 'var(--color-link)' : 'var(--color-icon)'}
        />
      </ToolbarButton>
    </Tooltip>
  )
